...
%% Last-Updated: 2019-02-23T19:23:27+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\item \label{item:reparametrization}Formula~\eqref{eq:koopman-pitman} is
  obtained from the latter one by the one-to-one reparametrization
\begin{equation}
...
\end{equation}
This equation cannot be solved explicitly for $\yl$ in terms of $\yc$. The
parametrization in terms of $\yl$ has several special properties:
\begin{enumerate}[label=\alph{enumi}.\arabic*.]
\item The quantities $(\yll_{r})$ can assume any values
  independently of one another, whereas the limit statistics $\yc$ have
  interdependent ranges.
...
Remark \ref{item:reparametrization} shows that the absolute value of each
parameter, $\abs{\yll_{r}}$, somehow quantifies the degree of relevance of
the corresponding statistic, zero meaning complete irrelevance. These
absolute values can therefore be used as parameters for our continuum of
hypotheses about the sufficient statistics. The \dob\ we ultimately want to
quantify is thus
\begin{equation}
