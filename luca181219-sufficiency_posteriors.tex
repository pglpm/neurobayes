...
%% Last-Updated: 2019-02-24T18:06:11+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{subequations}
  ...
  \end{gather}
\end{subequations}
The last expression defines $r$ functions $c_{r\ys}(\yl)$ of the sample
activity $\ys$ \emph{and of $\yl$} that have a role analogous to the
statistic $C_{r\yS}$ of the full-population activity $\yS$, in the sense
that
\begin{equation}
...
\begin{equation}
  ...
  C_{r\yS} \defd \binom{\yS}{r}\;\binom{N}{r}^{-1}
...
possible ones. Analogous statistics $\binom{\ys}{r}\;\binom{n}{r}^{-1}$ can
...
\begin{equation}
  ...
  \sum_{\ys} \binom{\ys}{r}\;\binom{n}{r}^{-1}\;p(\ys)
=   \sum_{\yS} \binom{\yS}{r}\;\binom{N}{r}^{-1}\;p(\yS),
\end{equation}
no matter what the \dobs\ about full-population and sample might be,
provided that they are related by $p(\ys) = G_{\ys\yS}\;p(\yS)$.

Note, however, that given a \emph{specific} $p(\yS)$ and any statistic
$C_{r\yS}$ it is always possible to create a function $c_{r\yS}$ of the
sample activity that satisfies
\begin{equation}
...
\end{equation}
But note that this function \emph{depends on the specific p(\yS)}.
...
\begin{equation}
  ...
  \sum_{\ys} \binom{\ys}{r}\binom{n}{r}^{-1} f_{\ys}
  \approx
  \sum_{\ys} c_{r\ys}(\yl) f_{\ys},
\end{equation}
