...
%% Last-Updated: 2019-02-23T23:43:50+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\item our \dob\ about the sequence $\ySt$ depends only on the frequencies
  $\yF$ of that sequence;
\item if $\pf(\yl \| \yI)$ is constant, the mode of our
  \dob~\eqref{eq:posterior_given_full_population} about $\yl$ can be calculated by
  minimizing with respect to $\yl$ the relative entropy between $\yF$ and
  $\pf(\yS \| \yl, \yI)$, leading to the maximum-entropy equations for $\yl$.
\end{enumerate*}
...
\begin{equation}
  ...
  \pf(\yst \| \yl, \I).
\end{equation}
We can calculate the last factor by introducing $\ySt$ and using the
hypergeometric distribution~\eqref{eq:dob_sample_given_pop}, the
Koopman-Pitman formula~\eqref{eq:koopman-pitman}, and introducing the
relative frequencies $\yf$ of the sample activities:
\begin{equation}
...
The maximum of the expression above with respect to $\yl$ is again found by
minimizing a relative entropy. If we try to find such minimum by taking the
partial derivatives with respect to $\yll_{r}$ we arrive at the following
system of equations:
\begin{subequations}
  ...
  \end{gather}
\end{subequations}
The last expression defines $r$ functions $c_{r\ys}$ of the sample activity
$\ys$ that have the same role as the statistic $C_{r\yS}$ of the
full-population activity $\yS$, in the sense that
\begin{equation}
