...
%% Last-Updated: 2019-03-31T18:59:35+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
information is shown in \fig~\ref{fig:mutual_info_histo_unif}. It tells us
that the response is likely to be informative; the most likely values of
the long-range mutual information are around $0.2\,\textrm{bit}$. The next
section explains intuitively why this inference is correct.
\begin{figure}[h!]
...
pairs with similar conditional frequencies; for example
\begin{equation}
...
\fig~\ref{fig:mutual_info_histo_cons}. The estimate now says that we should
...
sample $5\,000$ pairs uniformly. Let's now plot the samples by showing the
probability that each assigns to our observed data on the horizontal axis,
and their associated mutual information on the vertical axis. We obtain the
scatter plot of \fig~\ref{fig:init_scatter_unif}.
\begin{figure}[b!]
...
Now consider the conservative initial density~\eqref{eq:credib_cons}.
Figure~\ref{fig:init_scatter_cons} shows $5\,000$ samples from it, plotted
as in the previous case. The majority of those that give high probability
to the data have a negligible mutual information. This is reflected by the
estimate of \fig~\ref{fig:mutual_info_histo_cons}. \mynote{Need new scatter
plot}
\begin{figure}[b!]
\centering\includegraphics[width=0.75\linewidth]{A_init_scatter.png}\\
