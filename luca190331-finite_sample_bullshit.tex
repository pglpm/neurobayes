...
%% Last-Updated: 2019-03-31T23:02:03+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{The `sampling bias' is not a bias%\\{\large ***}%
}
...
\begin{abstract}\labelsep 0pt%
  \noindent It is shown that the so-called \enquote{sampling bias} in the
  estimation of the mutual information for stimulus-response frequencies
  is not a bias at all. If the estimate from the sample forecasts a high
  mutual information, then we response is indeed very likely to be
  informative. On the other hand, if we expect the response to be
  uninformative or if we just want to be conservative, a correct
  calculation of the estimate leads to a negligible mutual information.\par
  In either case no corrections of any kind are needed.%
...
information for long-run frequencies from a small sample is subject to a
bias, a systematic error. Other works have proposed various corrections to
this alleged error.

In this note I show that no such bias exists, and the estimates are
reliable. On the one hand, if we are  equally uncertain about the long-run
response frequencies, and the estimate from a small sample suggests a large
mutual information, then we should indeed expect the response to be
informative. On the other hand, if we  initially suspect that the
long-run response frequencies should be uninformative, or if we are very
conservative in our inference, then a correct calculation leads to very low
estimates of the mutual information. As a consequence no corrections of any
kind are needed for the estimates.

I show this first with a quick calculation in the next section, and then
with a longer reasoning in §***. The final section discusses what kind of
incorrect reasoning leads to the idea that there's a bias.

The calculations use the example given by Panzeri \etal\
\citey[Fig.~1]{panzerietal2007}: the stimulus can assume two values, the
response ten values.

\mynote{Comment on \enquote{sampling bias}: it has a different meaning in statistics}

\section{Bias? No: Bayes}
...
stimulus and response. We assume that all stimuli appear with the equal
...
example, in the inference of genetic peculiarities from diseases (or
phenotype in general) and vice versa. Imagine that we know the relative
frequencies of two diseases among people have a particular variant of a
gene, and the relative frequencies among people having another variant. Now
we ask: given that a specific person has one of the diseases, what's the
probability they have a particular gene variant? The answer is that we
don't know until we specify the proportions of individuals having the two
gene variants or the diseases. Because, even if the disease occurs more
frequently for one genetic variant, the number of people having such
variant may be so small that the probability still favours the other
variant. It's the well-known importance of incidence rates.

For this reason the result we obtained is not connected to a choice between
Bayesian or frequentist mindsets, even if we used Bayes's theorem in
\sect~\ref{sec:bias_bayes}. A serious frequentist statistician would reach
the same conclusions.

\medskip

The erroneous conclusions reached in Panzeri \etal\ \citey{panzerietal2007}
stem from a combination of factors. First of all the analysis given there
is topsy-turvy: our problem is not to infer data from a specific known
frequency, but to infer an unknown frequency -- among several possible ones
-- from known data. The second, related factor is the focus on one particular
frequency only at a time: what matters, instead, is the relative
probabilities that the data have under several candidate frequencies. The
third factor
