...
%% Last-Updated: 2020-07-12T20:01:55+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
frequencies from the sample and their small-overlapping supports). The
...
\item In some cases the spread (say, the inner 95\% interval) of the
  ...
  example 18\% for Case D. In other cases the spread is over a very broad
  range compared to the maximal possible one, for example 53\% for Case C.
...
To check whether they really are representative, we therefore would need to
know the true ones. And we don't -- this was indeed the problem we started
from. Such \enquote{validations} are therefore logically circular and
...
proportional to a multinomial coefficient. \mynote{will write the exact
  formula}
...
generally be expected more often that high values, owing to biological
...
Let's proceed as follows. Given a superdistribution, we choose from it two
...
\caption{Example of guess for long-run mutual information from the mutual
  ...
  superdistribution. If the sample mutual info is $0.2\:\bit$ ( yellow
  dashed horizontal line), then the long-run mutual info is in this case
  inferred to be at 95\% between $\clcl{0.097, 0.39}\:\bit$ and at 68\%
  between $\clcl{0.15, 0.30}\:\bit$ (yellow dashed vertical lines), with a
  median of $0.22\:\bit$, not far from the sample one \parencite[compare
  with the conclusions in][Fig.~1A]{panzerietal2007}.}\label{fig:backward}
\end{figure}% finite_sampling_bullshit.R
% 50\%: 0.21573429
...
circular and meaningless. Such simulation depends on a choice either of
...
\mynote{The long-run frequencies of the two kinds of stimulus are also very
