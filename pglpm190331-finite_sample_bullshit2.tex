...
%% Last-Updated: 2020-07-11T17:53:18+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
often. From the long-run\footnote{\enquote{But this \emph{long run} is a
    ...
    dead.} \citep[\sect~3.I, p.~65]{keynes1923_r2013}} joint frequencies of
...
possible, we plot some possible results for the sample mutual information
in the bottom panel. The long-run mutual information is indicated by the
red line. The sampled mutual information is most surely always higher than
the long-run one, with an average of $0.2\:\bit$.
...
\end{figure}% finite_sampling_bullshit.R
\textbf{Case B}. Suppose the long-run frequencies of responses conditional
on each kind of stimulus are as given in the top panel of
\fig~\ref{fig:resp_caseB}. The responses do not occur equally often in the
long run, and the frequencies of some of them are different for the two
kinds of stimulus. The long-run mutual information is $0.48\:\bit$.

We again consider a sample of 20 response measurements for each stimulus.
The corresponding plot for the sampled mutual information is shown in the
bottom panel. The sampled mutual information is almost symmetrically
distributed around  the long-run value, with an average of $0.51\:\bit$.
...
\end{figure}% finite_sampling_bullshit.R
\textbf{Case C}. Suppose the long-run frequencies of responses conditional
on each kind of stimulus are as given in the top panel of
\fig~\ref{fig:resp_caseC}. Also in this case the responses do not occur
equally often in the long run, and their frequencies are different for the
two kinds of stimulus. The long-run mutual information is $0.92\:\bit$.

We again consider a sample of 20 response measurements for each stimulus.
The corresponding plot for the sampled mutual information is shown in the
bottom panel. The distribution of the sampled mutual information is very
skewed, with an average $0.97\:\bit$. The distribution's support is almost
divided in distinct blocks; this is the result of the discreteness of the
possible values of the frequencies from the sample (they must be multiples
of $1/20$), together with the extreme separation of the long-run frequency
distributions for the two stimuli.
