...
%% Last-Updated: 2018-12-31T13:18:53+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
plausibility model, also called \emph{extremal} or independent, and the
relationship between the two. This relationship, extensively studied by
Lauritzen
...
$(x_1, x_2, \dotsc) \defs \yx$, $(t_1, t_2, \dotsc) \defs \yt$ modifies
our \dob\ about a new output datum $x$ conditional on a new input datum $t$:
\begin{equation}
...
\begin{multline}
  ...
  \pf(x_2 \| x_3, \dotsc,\,  t_2, t_3, \dotsc,\, \yIl) \times
    \dotsb
    \\
  &\ne \pf(x_1 \| t_1,\, \yIl) \times
  \pf(x_2 \| t_2,\, \yIl) \times \dotsb.
  \end{aligned}
\end{multline}
With an extremal model $\yIe$, knowledge about other data doesn't affect our \dob:
\begin{equation}
...
\begin{equation}
  ...
  = \pf(x_1 \| t_1,\, \yIe) \times
  \pf(x_2 \| t_2,\, \yIe) \times \dotsb.
\end{equation}

Why is the latter kind of model called \enquote{extremal}?
...
$\pf(\yx \| \yt,\, \yw, \yI)$ labelled by a parameter $\yw \in \yws$, the
weights of the combination being $\pf(\yw \| \yI)$. Each labelled density
$\pf(\yx \| \yt,\, \yw, \yI)$ is an extremal model $(\yw,\yI)$, as clear
...
given as a convex combination of extremal models. Most important, this
convex combination is \emph{unique}: if we change the family of extremal
models or the weight density $\pf(\yw \| \yI)$ the resulting learning model
will assign different plausibilities to some data sets $(\yt,\yx)$. This
means that a learning model uniquely selects a family of extremal,
non-learning models and a weight density.

By keeping the family of extremal models
$\set{(\yw, \yI) \suchthat \yw \in \yws}$ fixed and considering all
possible weight densities we generate a space of plausibility models. By
construction this is a \emph{convex set}, and its extremal points (a point
being a plausibility model) are non-learning models of the
form~\eqref{eq:integral_representation_factorization}. This is why they are
called \enquote{extremal}. The extremal model labelled by $\ywp$
corresponds to a singular convex combination with a delta weight density
$\delt(\yw-\ywp)$. [***refs for convex spaces]
...
\end{equation}
yielding a new point in the convex set; but the extremal points are
unchanged. As the training data accumulate, the weight density become very
