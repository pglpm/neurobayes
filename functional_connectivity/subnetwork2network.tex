...
%% Last-Updated: 2019-01-25T19:14:06+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Given sample, infer population}
...
population. By inference we mean the numerical evaluation of our \dob\
about the population's state. We consider the state at one instant of time.
The state will be taken to be the binarized activity from a time-binned
sequence; but our discussion shall apply to more general definitions of
\enquote{state}. Denote by $N$ the size of the population, by $n$ that of
the sample, by $S_i$ the activity of the $i$th neuron in the population, by
$\yS \defd (S_1, \dotsc, S_N)$ the joint activity -- the state -- and by
$s_j$, $\ys \defd (s_1, \dotsc, s_n)$ the corresponding activities and
state of the sample.

Let's make some additional remarks to better understand what our inference
is about.

First, our inference is not about the dynamics of the population. In the
problem of inferring the dynamics, we assume that the state $\yS(t)$ at
time $t$ is determined through a dynamical law by the state $\yS(t-\ydt)$
at time $t-\ydt$ together with some external quantities $Q(t-\ydt)$:
\begin{equation}
  \label{eq:determ_dynamics}
  \yS(t) = F[\yS(t-\ydt), Q(t-\ydt)],
\end{equation}
but we are uncertain about the mathematical form of the dynamical law $F$
and the values of the external quantities $Q(t-\ydt)$. For this reason we
can express our \dob\ about $\yS(t)$ given only our knowledge about
$\yS(t-\ydt)$ and other initial assumptions $\yI$:
\begin{equation}
  \label{eq:prob_dynamic}
  \pf[\yS(t) \| \yS(t-\ydt), \yI],
\end{equation}
and, given a sequence of states, our \dob\ about the dynamical law:
\begin{equation}
  \label{eq:prob_dynamical_law}
  \pf[F \| \yS(t_1), \yS(t_2), \dotsc, \yI].
\end{equation}
Our present problem doesn't concern this inference of the dynamics, but is
very relevant to it, because to infer the dynamics we must infer the
population states $\yS(t_k)$ first, from the observation of a population sample.
