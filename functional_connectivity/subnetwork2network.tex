...
%% Last-Updated: 2019-01-26T12:15:25+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
The state is taken to be the binarized activity from a time-binned
sequence. Denote by $N$ the size of the population, by $n$ that of the
sample, by $S_i$ the activity of the $i$th neuron in the population, by
$\yS \defd (S_1, \dotsc, S_N)$ the joint activity -- the state -- of the
population, and by $s_j$, $\ys \defd (s_1, \dotsc, s_n)$ the corresponding
activities and state of the neurons making up the sample. Many points of
our discussion, however, apply to more general definitions of
\enquote{state}.
...
about some previous states and other initial assumptions $\yI$:
\begin{equation}
...
\begin{equation}
  ...
  \pf[F \| \set{\yS(\yt)}, \yI].
\end{equation}
Our present problem doesn't concern this kind of inferences, but is very
relevant to them: to infer the dynamics,
\eqn~\eqref{eq:prob_dynamical_law}, we usually must first infer the
population states $\yS(\yt)$ from the observation of a population sample.
...
therefore refer to one time $t$ only, which is conveniently suppressed from
our notation.
