...
%% Last-Updated: 2019-01-29T14:12:04+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
$\set*{\pf(\ym \| \yIme')}$, $\set*{\pf(\ym \| \yIme'')}$ for $\ym$ may yield
the same resulting distribution for $\yS$. Mixtures of maximum-entropy
distributions like~\eqref{eq:initial_dob_me_mixture} make sense when we are
making inferences about an \emph{unlimited} sequence of states -- for
example, a time sequence $\bigl( \yS(t_1), \yS(t_2), \dotsc \bigr)$ --
because in this case each set of weights for $\ym$ gives rise to a unique
distribution \emph{for the unlimited sequence}.
...
recording instruments is usually
