...
%% Last-Updated: 2018-06-26T00:16:57+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
Lorentz transformations and special relativity, or Noether's theorem
\citep{noether1918}[anticipated for the Euclidean group
by][]{cosseratetal1909}. \mynote{mention also Bertrand's problem and}
\citep{jaynes1973b}
...
outcomes in a set $K$. We represent the proposition \enquote{The $i$th
  observation yields outcome $k$} by $\yO{i}{k}$, with $k\in K$.
...
\begin{equation}
  ...
  \p(\yO{m+n}{k_{m+n}}, \dotsc, \yO{m+1}{k_{m+1}}
  \| \yO{m}{k_{m}}, \dotsc, \yO{1}{k_{1}}, \yI),
\end{equation}
where the comma represents the conjunction (\enquote{$\land$}) of the
propositions. We will also use the simplified notation
$\pf(k_{m+n}, \dotsc, k_{m+1} \| k_{m}, \dotsc, k_{1}, \yI)$.

In general the full set of observed data $(k_{1}, \dotsc, k_{m})$ is
important for this inference. But in particular situations it may
happen that only specific properties of the data are \emph{sufficient} to our
inference, the remaining details being \emph{irrelevant}. By
\enquote{irrelevant} we mean that our inferences aren't changed if we
forget about those details or if we don't know them in the first place.
Let's represent the relevant properties by a map $\ys(k_{1},\dotsc,k_{m})$,
called a \emph{sufficient statistic}, with values in some space $S$. The
relevance and irrelevance are expressed by the equalities
\begin{multline}
  \label{eq:predictive_sufficiency}
  \pf(k_{m+n}, \dotsc, k_{m+1}
  \| k_{m}, \dotsc, k_{1}, \yI)
  =
  \pf[k_{m+n}, \dotsc, k_{m+1}
  \| \ys(k_{1}, \dotsc, k_{m}), \yI]
  \\[\jot]
  \text{for all $m$, $n$, and $(k_{i})$.}
\end{multline}

Typical examples of sufficient statistics are the number of known
observations $m$ together with their mean $(k_{1} + \dotsb + k_{m})/m$, or
second moments $(\yk_{1}{\yk_{1}}\T + \dotsb + \yk_{m}{\yk_{m}}\T)/m$, or
range $\max_i(k_{i})-\min_i(k_{i})$; for example
\begin{equation}
  \label{eq:example_mean_suffstat}
  \ys(k_{1}, \dotsc, k_{m}) \defd
  \Bigl( m,\; \tfrac{1}{m}\tsum_i k_{i} \Bigr).
\end{equation}
...
\citep{koopman1936,pitman1936,darmois1935}[see also later
analyses:][]{hipp1974,andersen1970,denny1967,fraser1963,barankinetal1963}\mynote{add
  comments about these references and about the discrete-variable case}
