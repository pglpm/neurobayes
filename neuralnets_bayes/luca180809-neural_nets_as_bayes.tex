...
%% Last-Updated: 2019-01-18T13:02:15+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
$\pf(\yp \| \yD, \yM)$ about a set of propositions $\set{\yp}$ given data
$\yD$ and a model $\yM$. The propositions often concern the happening of
events or the values of physical quantities. The model is the set of
numerical \dobs\ that are sufficient to arrive at $\pf(\yp \| \yD, \yM)$
using the plausibility calculus.
...
\begin{equation}
  ...
  \text{choose}\quad
  \argmax_{\ya} \sum_{\yp} \yg(\ya \| \yp, \yM)\; \pf(\yp \| \yD, \yM).
\end{equation}
...
problem is metonymically represented by the choice of an event or quantity,
but the intended actions are different: we say \enquote{choose $\yp$} but
we mean \enquote{behave as if $\yp$ were true}. For example, the uncertain
events $\set{\yp}$ are
...
$\set{\text{\enquote{take the umbrella}}, \text{\enquote{don't take the
      umbrella}}}$. It's important to be aware of this difference. Often
the literature presents problems that look like inferences but are actually
decisions. The \enquote{choice of a parameter} is an example of this
ambiguity. This point will be important for our Bayesian interpretations of
machine-learning procedures. In relation to this, it's remarkable that
often a lot of effort is spent in the literature on the numerical choice
and determination of \dobs, but little is spent on the choice of the gain
function. From formula~\eqref{eq:expected_gain} it's clear that the
solution of the decision problem depends equally on both.

Two models $\yM'$, $\yM''$ can lead to different inferences and yet lead to
the same decisions if
\begin{equation}
  \label{eq:equivalent_decision_models}
  \sum_{\yp} \yg(\ya \| \yp, \yM')\; \pf(\yp \| \yD, \yM')
  =
  \sum_{\yp} \yg(\ya \| \yp, \yM'')\; \pf(\yp \| \yD, \yM'')
  \quad\text{for all $\ya$, $\yD$}.
\end{equation}
For example, when $\pf(\yp \| \yD, \yM') = \pf(\yp \| \yD, \yM'')\;h(\yp)$
and $\yg(\ya \| \yp, \yM') = \yg(\ya \| \yp, \yM'')/h(\yp)$. It's therefore
impossible to ascertain the \dobs\ determined by a model if we're only
given the decisions ensuing from it. This point will also be important in
