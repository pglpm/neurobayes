...
%% Last-Updated: 2019-04-06T20:15:15+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
recorded sample activities with relative frequencies $\yf$:
\begin{equation}
  \label{eq:posterior_given_sample}
  \pf(\yl \| \yf, \yI) \propto \pf(\yl \| \yI)\;
  \pf(\yf \| \yl, \yI).
\end{equation}

First we note that our \dob\ about every sequence $\yst$ having frequencies
$\yf$ have the same value, under the hypothesis of sufficiency \emph{for
  the full population} and assuming the long-run statistics are known --
that is, with $\yl$ given:
\begin{equation}
  \label{eq:prob_particular_sequence}
  \pf(\yst \|\yl, \yI) = \prod_t \pf(\ys_{t} \| \yl, \yI)
  = \prod_{\ys} \pf(\ys \| \yl, \yI)^{T f_{\ys}}.
\end{equation}
The joint \dob\ factorizes because the \dob\ for each $\ys_{t}$ is
...
Our \dob\ $\pf(\ys \| \yl, \yI)$ about the sample activity at any
particular time, given $\yl$, can be obtained by marginalizing the
corresponding $\yS$:
\begin{equation}
...
activities is then proportional to the number of sequences having those
frequencies:
\begin{equation}
  ...
  \begin{split}
    ...
    \Bigr\}
  \end{split}
% \pf( \yst \|\yl, \yI) =\sum_{\ySt} \pf(\yst \| \ySt, \yI)\;\pf(\ySt \| \yI)
% =    \int\!\di\yl\; \pf(\yl \| \yI)\; \prod_{t}\pf(\ys_{t} \| \yl, \yI)
