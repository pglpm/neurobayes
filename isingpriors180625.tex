...
%% Last-Updated: 2018-06-27T20:06:00+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
and $\yt\defd(m_1, m_2,l)$ into one-one
...
\begin{equation}
  ...
  0\le m_1,m_2 \le 1,
  \qquad
  \max(0,m_1 + m_2 -1) \le l \le \min(m_1,m_2)
\end{equation}
...
%  \equiv{}\\
  \frac{\di\yt}{l\,(m_1-l)\,(m_2-l)\,(1+l-m_1-m_2)},
\end{equation}
...
\begin{multline}
  \label{eq:conjecture}
  \det\biggl[ \frac{\de^2\ln Z(\bm{\theta})}{\de\theta_k\,\de\theta_l} \biggr]
  = \prod_{\bm{s}\in\set{0,1}^n}
  \frac{\exp\bigl(\sum_{i=1}^n h_i s_i + \sum_{1\le i < j\le n} J_{ij}s_i s_j\bigr)}{Z(\bm{\theta})}
  \\[2\jot]
  \text{with}\quad
  Z(\bm{\theta}) \defd \sum_{\bm{s}} \exp(\dotso),\quad
  \bm{\theta}\defd (\theta_k) \defd (h_i, J_{ij})
\end{multline}
