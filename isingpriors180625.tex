...
%% Last-Updated: 2018-06-27T00:52:26+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\end{equation}
We give first an intuitive understanding of this result and then a
mathematical argument (no proof) for it.


Consider the parameterized, factorized joint probability
...
\end{multline}
owing to the properties of the expectation and the factorized form of the
joint probability conditional on $\yth$. From the properties of the
variance we also have
\begin{equation}
...
This means that for large $N$ and each triplet $\yth$ we approximately have
a probability distribution for the statistics very peaked on particular
values $\yt \defd (m_1,m_2,l)$ determined by the equations
\begin{equation}
  ...
  \end{gathered}
\end{equation}
This system of equations actually puts into one-one correspondence
\citep{meadetal1984} the parameters $\yth \defd(\mu_1, \mu_2, \la)$ and
$\yt\defd(m_1, m_2,l)$. The former belong to $\RR^3$; the latter to the
bounded domain
\begin{equation}
...
\end{equation}
Using these new parameters we therefore have, for large $N$,
\begin{equation}
...
\begin{gather}
  ...
  l = \frac{\de\ln Z(\yth)}{\de\la}
    \equiv \expe{s_1 s_2\|\yth,\yI}.
  \end{gathered}
  \\[2\jot]
  \label{eq:max_params}
  \text{for}\qquad
  (m_1,m_2,l) = (\yavv_1,\yavv_2,\ycv),
  \quad \yth=\ythm.
\end{gather}
The system of equations~\eqref{eq:change_coords} gives a one-one
correspondence between the parameters $\yt\defd(m_1, m_2,l)$ and
...
\section{Inference from observed data}
\label{sec:inference_from_data}
