...
%% Last-Updated: 2018-06-26T14:22:42+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\headauthor}{\ifpublic Roudi, Porta Mana%
\else Luca\fi}
...
\hspace*{\stretch{1}}\parbox{0.5\linewidth}%\makebox[0pt][c]%
...
Assume that we have $m$ observations $(\yso{1}, \dotsc, \yso{m})$ of this
...
\begin{equation}
  ...
  m,\qquad
  \tfrac{1}{m}\bigl( \yso{1} + \dotsb + \yso{m} \bigr) \defs \yav,
  \qquad
  \tfrac{1}{m}\bigl( \ysso{1}_1\ysso{1}_2 + \dotsb + \ysso{m}_1 \ysso{m}_2 \bigr)
  \defs \ycv
\end{equation}
are sufficient statistics; note that the second sum contains the
first as its diagonal. These assumptions are collectively denoted $\yI$.
Then the Koopman-Pitman theorem says that our probabilistic forecasts must
assume this general form:
\begin{multline}
...
\end{multline}
For later convenience, denote $\yth \defd (\mu_1, \mu_2, \la)$.
...
determined by symmetry or combinatorial properties of the system.
% for example, if each unit actually represents the total activity of a population of sub-units.
In the present study we assume it to be unity: $g(\ys)=1$. The density
$\pf(\yth \|\yI)$ is called \emph{prior parameter density}.

Geometrically, the formula above says that our probability distribution is
the convex combination of probability distributions from a
three-dimensional family parameterized by $\yth$, with weights
$\pf(\yth \|\yI)\,\di\yth$. The parameters $\yth$ are just coordinates of
this three-dimensional manifold, and we can choose other coordinates $\yt$,
the weights then being given by $\pf(\yt \|\yI) \, \di\yt$, with the
densities for $\yth$ and for $\yt$ related by a Jacobian determinant:
\begin{equation}
  \label{eq:jacobian}
...
\begin{subequations}    \label{eq:conditional_prediction}
  \begin{gather}
    \begin{multlined}[][0.9\linewidth]
      ...
      \,
      % \times{}\\ \shoveright{
      \pf(\yth \| \yso{1},\dotsc,\yso{m} \yI) \, \di\yth
      % }
    \end{multlined}
    \\
    \shortintertext{with}
    \begin{multlined}[0.95\linewidth]
    \pf(\yth \| \yso{1},\dotsc,\yso{m} \yI)
    \propto
    \Biggl[  \prod_{i=1}^{m}
    \frac{  \exp\bigl(
      \mu_1 \ysso{i}_1 + \mu_2 \ysso{i}_2 + \la \ysso{i}_1 \ysso{i}_2
...
={}\\[\jot]
  ...
  \end{gather}
\end{subequations}
The density $\pf(\yth \| \yso{1},\dotsc,\yso{m} \yI)$ is called
\emph{posterior parameter density}.

The last expression shows that the $m$ observations affect our forecast
...
consider populations with many units. When the number $m$ of known
observations is large, the posterior parameter density is often
approximated by a Dirac delta centred on the maximum of the argument of the
exponential, $\mu_1 a_1 + \mu_2 a_2 + \la \ycv$.
