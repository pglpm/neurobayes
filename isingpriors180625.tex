...
%% Last-Updated: 2018-07-20T15:58:19+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
Consider then a new set of parameters $\yq\defd(q_1,q_2,\yql)$ defined as
\begin{equation}
  \label{eq:params_q}
...
The parameter transformation above has unit Jacobian determinant, so a
constant parameter density in $\ym$ is also constant in $\yq$. The latter
is a special case of a parameter density function used by Laplace
\citey{laplace1778_r1781} and motivated by Johnson
\citey{johnson1924,johnson1932c}, which uses Dirichlet's
\citey{dirichlet1839} integral
...
\text{with}\quad q_0 \defd 1-q_1-q_2-\yql, \qquad \yN>0, \ynn \in \simpl,
\end{gathered}
\end{equation}
where $\simpl$ is the three-dimensional simplex
\begin{equation}
\simpl \defd \set{\bm{x} \in \RR^4 \suchthat 
  \tsum_{s=1}^4 x_s=1, x_s\ge 0}
\label{eq:simplex}
\end{equation}
to which also $(q_0,\dotsc,q_3)$ belong. This density is commonly called
\enquote{Dirichlet prior}, but we'll call it the Johnson density here,
since he determined its mathematical form from informational desiderata.

The Johnson density is a conjugate prior
\cites[\chap~9]{degroot1970_r2004}{diaconisetal1979b}: it updates to a
density of the same mathematical form. It leads to probabilities that can
be analytically calculated using Dirichlet's formula
\begin{equation}
...
The constant density $\pf(\yt \| \yIt)\,\di\yt = 6\,\di\yt$ is a special
case of the Johnson density with $\yN=4$, $\ynn_i=1/4$. 
