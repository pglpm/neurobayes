...
%% Last-Updated: 2018-06-26T23:45:36+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
In this section we show that there is a particular coordinate system $\yt$
of the three-dimensional manifold discussed above for which \emph{the prior
  parameter density coincides, in the large-$N$ limit, with the probability
  of the observed statistics}:
\begin{equation}
  \label{eq:special_parameter_is_statistics}
  \pf[(\yav,\ycv)=\bm{x} \| \yI, \text{large $N$}]
...
Consider the parameterized joint probability
$\pf(\yso{1}, \dotsc, \yso{N} \|\yth, \yI )$ of
\eqn~\eqref{eq:component_combination}, noting that it can be written
\begin{equation}
...
\begin{gather}
  ...
  \quad \yth=\ythm.
\end{gather}
The system equations~\eqref{eq:change_coords} gives a one-one
correspondence between the parameters $\yt\defd(m_1, m_2,l)$ and
$\yth \defd(\mu_1, \mu_2, \la)$. The latter belong to $\RR^3$, whereas the
former belong to the bounded domain
\begin{equation}
